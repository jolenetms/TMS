...
% Import the data
HAMD_by_week = readtable("C:\Users\ins4004\Documents\Projects\Response_Trajectories_Project\THREED_Data_Analyses\THREE-D HRSD_IDS_QIDS Total Data.csv", opts);
HAMD_by_week_array = table2array(HAMD_by_week(:, 2:end));
...
meetsCriteria = false(size(HAMD_by_week_array)); % Assuming 372 rows
 allHAMD_Delta = zeros(size(HAMD_by_week_array)); 
 sg_reversalvalue = [];
...
remission_criteria_met = false(size(HAMD_by_week_array, 1), 7);

% Loop through each participant and extract their data
for p = 1:size(HAMD_by_week_array, 1)
    participant_data = HAMD_by_week_array(p, :);
...
% Extract the row (ppt) and column (week) that remission was reached for each instance of remission
...
for sessionindex = 3:17
    ...
    HAMD_Delta =  HAMD_by_week_array(:, sessionindex + 1) - HAMD_by_week_array(:, sessionindex);

    allHAMD_Delta(:, sessionindex) = HAMD_Delta;

    halfdelta = allHAMD_Delta./2; %divide the change in HAMD by 2

    reversalvalue = HAMD_by_week_array + halfdelta;

    % Calculate the percentage differences
    HAMD_percentdecrease =  (HAMD_by_week_array(:, sessionindex) - HAMD_by_week_array(:, sessionindex + 1)) ./ HAMD_by_week_array(:, sessionindex) * 100;

    % Extract 3 Pre and 3 Post HAMD values from gain
    if sessionindex == 3
         HAMD_pre = HAMD_by_week_array(:, (sessionindex - 1):(sessionindex)); 
    else 
        HAMD_pre = HAMD_by_week_array(:, (sessionindex - 2):(sessionindex)); 
    end

    if sessionindex == 17
         HAMD_post = HAMD_by_week_array(:, (sessionindex + 1):(sessionindex + 2)); 
    else 
        HAMD_post = HAMD_by_week_array(:, (sessionindex + 1):(sessionindex + 3));
    end  
    ...
    allsymptomfluctuation(bye_rows,:) = NaN;
        ...
        for i = 1:length(PrePost_NaN_rows)
            ...
            symptomfluctuation = 4.303 * sqrt(((HAMD_by_week_array(PrePost_index, sessionindex) - 1) .* std_pre(PrePost_index).^2 + (HAMD_by_week_array(PrePost_index, sessionindex + 1) - 1) .* std_post(PrePost_index).^2) ./ (HAMD_by_week_array(PrePost_index, sessionindex) + HAMD_by_week_array(PrePost_index, sessionindex + 1) - 2));
        ...
        for j = 1:length(One_NaN_rows)
            ...
            symptomfluctuation = 3.182 * sqrt(((HAMD_by_week_array(OneNaN_index, sessionindex) - 1) .* std_pre(OneNaN_index).^2 + (HAMD_by_week_array(OneNaN_index, sessionindex + 1) - 1) .* std_post(OneNaN_index).^2) ./ (HAMD_by_week_array(OneNaN_index, sessionindex) + HAMD_by_week_array(OneNaN_index, sessionindex + 1) - 2));
        ...
        symptomfluctuation = 2.776 * sqrt(((HAMD_by_week_array(zero_index, sessionindex) - 1) .* std_pre(zero_index).^2 + (HAMD_by_week_array(zero_index, sessionindex + 1) - 1) .* std_post(zero_index).^2) ./ (HAMD_by_week_array(zero_index, sessionindex) + HAMD_by_week_array(zero_index, sessionindex + 1) - 2));
...
consolidatedreversal = false(size(HAMD_by_week_array));
...
reversal_ppt = HAMD_by_week_array(rowIndex(i), indices) >= sg_index(i,3);
...
variableNames = {'sg_ppt', 'remission_met_ppt', 'remission_met_week'};
...
HAMD_by_week = [HAMD_by_week, rows_with_ones_table];

HAMD_by_week = [HAMD_by_week, remission_criteria_met_atall_table];

HAMD_by_week = [HAMD_by_week, remission_criteria_week_table];

% Get the number of people in the table
TotalParticipants = size(HAMD_by_week, 1);

% Sudden Gain Analysis Comparison: Nili vs Us
our_sgsum = sum(HAMD_by_week.sg_ppt == 1)
% Nili_sgsum = sum(HAMD_by_week.all_crit == 1)
allour_gainers = (HAMD_by_week.sg_ppt == 1);
matching_gainers = all(HAMD_by_week.sg_ppt == 1 & HAMD_by_week.all_crit == 1, 2);

onlyour_gainers = all(HAMD_by_week.sg_ppt == 1 & HAMD_by_week.all_crit == 0, 2);
% onlyNili_gainers = all(HAMD_by_week.sg_ppt == 0 & HAMD_by_week.all_crit == 1, 2);
...
sum_remitted = sum(HAMD_by_week.remission_met_ppt == 1);

% Calculate the remission rate
remission_rate = (sum_remitted / 372) * 100;
% Find most common session for remission to occur

% Extract nonremitters from session count
nonZeroweeks = HAMD_by_week.remission_met_week(HAMD_by_week.remission_met_week ~= 0);
...
remission_week_percentage = (counts_vert ./ 72) * 100;
...
for personIndex = 1:TotalParticipants
    ...
    individual_HAMD = HAMD_by_week(personIndex, 2:8);
    %all_crit = HAMD_by_week.all_crit(personIndex);
    %sg_session_n = HAMD_by_week.sg_session_n(personIndex);
    %sg_ppt = HAMD_by_week.sg_ppt(personIndex);
    remission_met_ppt = HAMD_by_week.remission_met_ppt(personIndex);
    remission_week = HAMD_by_week.remission_met_week(personIndex);
    ...
    ID = HAMD_by_week.SubjectID(personIndex);
    ...
    if sg_ppt == 1 && remission_met_ppt == 1 % all_crit == 1
        ...
        title(['Subject ', num2str(ID), ' SG: ', num2str(sg_session_n), ' R: ', num2str(remission_met_week)]);
        xlabel('TMS Treatment Visit');
        ylabel('HAM-D Score');
...
% hold off;
    ...
    elseif sg_ppt == 0 && remission_met_ppt == 1 % all_crit == 1
        ...
        % Add labels, titles, legends, etc. to the figure
        title(['Subject ', num2str(ID), ' R: ', num2str(remission_met_week)]);
        xlabel('TMS Treatment Week');
        ylabel('HAM-D Score');
...
remission_met_1 = HAMD_by_week.remission_met_ppt(HAMD_by_week.remission_met_ppt ~= 0);
sg_ppt_1 = HAMD_by_week.sg_ppt(HAMD_by_week.sg_ppt ~= 0);

HAMD_by_week_array_all = table2array(HAMD_by_week);

% Find ppt who were both a remitter and sudden gainer
rem_and_sg_instances = all(HAMD_by_week.sg_ppt == 1 & HAMD_by_week.remission_met_ppt == 1, 2);
...
for sessionIndex = 2:19
    num_remaining_ppt(sessionIndex) = sum(~isnan(HAMD_by_week_array(:, sessionIndex)));
end
...
%         remission_crit1_sessionindex = HAMD_by_week_array(:, first_instance_under10(personindex));
%     else
